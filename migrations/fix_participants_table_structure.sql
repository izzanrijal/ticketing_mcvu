-- First, check if the registration_id column exists and create it if it doesn't
DO $$
BEGIN
  ...
  ELSE
    RAISE NOTICE 'registration_id column already exists in participants table';
  END IF;
END $$;

-- Now check if the foreign key constraint exists and create it if it doesn't
DO $$
BEGIN
...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'Index on participants.registration_id already exists';
  END IF;
END $$;
