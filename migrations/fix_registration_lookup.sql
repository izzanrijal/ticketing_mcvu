...
GROUP BY 
  r.id, r.registration_number, r.created_at, r.user_id, r.status, r.final_amount,
...
FROM registrations r
WHERE p.registration_id IS NULL 
AND p.id = 'aa880d3c-25fe-46e1-897d-ea1022c0fdea';
...
BEGIN
  ...
  UPDATE payments
  SET registration_id = reg_id
  WHERE registration_id IS NULL AND id = reg_id::text;
